...
pub struct WsgState {
    pub dim: (i32, i32),
    pub wolves_grid: SparseGrid2D<Wolf>,
    pub sheeps_grid: SparseGrid2D<Sheep>,
...
impl State for WsgState {
    fn reset(&mut self) {
        self.step = 0;
        self.wolves_grid = SparseGrid2D::new(self.dim.0, self.dim.1);
        self.sheeps_grid = SparseGrid2D::new(self.dim.0, self.dim.1);
...
impl WsgState {
    pub fn new(dim: (i32, i32), initial_animals: (u32, u32)) -> WsgState {
        WsgState {
            dim,
            wolves_grid: SparseGrid2D::new(dim.0, dim.1),
            sheeps_grid: SparseGrid2D::new(dim.0, dim.1),
